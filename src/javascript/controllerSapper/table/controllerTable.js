...
class ControllerTable {
    constructor() {
        ...
        if (!table)
            throw new Error("Не найден элемент с классом .board__table в файле controllerTable.js");
    ...
    click(event) {
        if (this.#isEndGame(modelTable.isEndGame))
            return;
    ...
    mouseDown(event) {
        ...
        if (this.#isEndGame(modelTable.isEndGame))
            return;
        if (modelTable.isActiveOrFlag(event.target))
            return;
