...
class ViewTable {
  ...
  createElementTable() {
    ...
    if (!board) {
      throw new Error(
        "Не найден элемент с классом .board в файле .vieTables.js"
      );
    }
  ...
  createTable(board: any) {
    let time = performance.now();
    let table = document.querySelector(".board__table");
    if (!table)
      throw new Error(
        "Не найден элемент с классом .table в файле viewTable.js"
      );
    for (let i = 0, maxTr = board.length; i < maxTr; i = i + 1) {
      ...
      for (let j = 0, maxTd = row.length; j < maxTd; j = j + 1) {
        ...
        if (cell.isOpen) console.log(123);
        tr.append(td);
      }
